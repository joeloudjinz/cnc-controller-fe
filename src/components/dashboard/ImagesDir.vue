<template>
  <v-card>
    <v-card-title class="teal white--text headline">User Directory</v-card-title>
...
export default {
  ...
  methods: {
    async fetchUsers(item) {
      ...
      return axios
        .get("http://localhost:3000/api/local/files/images")
        .then(result => {
          const imagesObj = result.data.Images;
          Object.keys(imagesObj).forEach(function(k) {
            item.children.push(imagesObj[k]);
          });
        })
        .catch(error => {
          console.warn(error);
